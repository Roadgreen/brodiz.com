...
export const ProductContextProvider = ({ children }: any) => {
 ...
 const [selectedProduct, setSelectedProduct] = useState({});
  ...
  const productSearch = async (doc:object, collection:string,db:string): Promise<[]> => {
    ...
    try{
      ...
      const reponseFindProduct = await fetch(
        process.env.FETCHPRODUCTSEARCH || "http://localhost:8080/product/productSearch",
        myInit
      );
...
return[]
  ...
  return (
    <ProductContext.Provider
      value={{
        productSearch,productArray,setProductArray,selectedProduct,setSelectedProduct
      }}
    >
      {children}
    </ProductContext.Provider>
