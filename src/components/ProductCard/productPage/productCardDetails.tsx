...
export default function ProductCardDetails({product}:{product:product}){
  ...
  const imgChange = (x:any) => {
    setImgSelection((prevSelection) => {
      ...
      return [newIndex, (newIndex + 1) % 6, (newIndex + 2) % 6, (newIndex + 3) % 6, (newIndex + 4) % 6, (newIndex + 5) % 6];
    });
  };
  const handleClick = (info: string) => {
    if (info.length > 2) {
      ...
      const newSizeChangeCss = size.map((x: any) => x.size === info ? styles.sizeOptionSelected : styles.sizeOption);
...
{product.img && product.img[0] ?    <div className={styles.imgsContainer}>  <div className={styles.photosContainer}>
    ...
    <div className={styles.descriptionContainer}>
        ...
        <div className={styles.sizeContainer}>
          <div className={styles.sizeDiv}>
            {size.map((x:any,i:number) => {
              return(
                <div className={sizeChangeCss[i]} onClick={()=>handleClick(x.size)} key={i}>{x.size}</div>
              )
            })}
          </div>
        </div>
        ...
        <button className={styles.buttonBuy} onClick={()=>{handleClickAddToCart()}}>Acheter</button>
    </div>
